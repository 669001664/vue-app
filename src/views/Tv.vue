<template>
  <div>
    <ul class="tv-list">
      <li
        class="tv-item"
        v-for="(item, index) in this.tvList"
        :key="index"
  ...
  methods: {
    getData() {
      if (this.isFinish) {
        ...
        this.axios
          .get(baseUrl + tvUrl)
          .then((res) => {
            this.tvList = this.tvList.concat(res.data.subject_collection_items); //把加载后新请求的数据连接进去
    ...
    listenScroll() {
      ...
      window.onscroll = () => {
        ...
        if (scrollTop + deviceHeight > fullHeight - 20 && this.isFinish) {
          this.start += 10;
          if (this.start < 50) this.getData();
        }
      };
    },
    getDetail(id) {
